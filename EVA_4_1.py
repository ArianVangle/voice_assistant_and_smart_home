...
def choose_command(text, abs=True):
	...
	for keys in commands:
		for i in range(0, len(commands[keys])):
			if fuzz.token_set_ratio(commands[keys][i], text) > 80 or fuzz.partial_token_set_ratio(commands[keys][i], text) > 80:
				print(commands[keys][i], fuzz.token_set_ratio(commands[keys][i], text),fuzz.partial_token_set_ratio(commands[keys][i], text))
	...
	if 'WIKIPEDIA' in command_list:
		...
		if len(command_list) > 1:
			...
			if fuzz.token_set_ratio('ева', text) != 100:
				try:
					command_list += ['GPT-4o']
					command_keys = 'GPT-4o'
					...
					voice_out('не могу получить доступ к нейросети, возможно нет подключения к интернету')
...
def light_control():
	...
	print(verbs)
	if 'impr' in verbs or 'включить' or 'выключить':
...
def open_browser():
	...
	if fuzz.token_set_ratio('найди', text) > 80:
		words = text.split()
		for w in words:
			if fuzz.token_set_ratio(w, 'браузере')>90:
				print(words)
				webbrowser.open(f'https://ya.ru/search/?text={"".join(words[2:])}')
				return 
			else: words.pop(0)
	else: webbrowser.open('https://ya.ru')
...
#------------------------------------------
	...
	if fuzz.token_set_ratio(text, 'повысь') > 90 or fuzz.token_set_ratio(text, 'увеличь') > 90 or fuzz.token_set_ratio(text, 'повысить') > 90:
		...
		voice_out(answer_text)
	if fuzz.token_set_ratio(text, 'понизить') >  90 or fuzz.token_set_ratio(text, 'уменьши') >  90 or fuzz.token_set_ratio(text, 'тише') >  90:
		volume.SetMasterVolumeLevelScalar(current_volume - 0.2, None)
...
def gas_sensor():
	...
	if rec != 0:
		...
		gas = n2t(int(rec.split()[5]), units_gas)
		if int(rec.split()[5]) > 600:
			voice_out(f'сейчас в воздухе порядка {gas} примесей, стоит проветрить')
		else: voice_out(f'сейчас в воздухе порядка {gas} примесей, показатели в норме')
	else:
		voice_out('не могу получить информацию с датчика')
